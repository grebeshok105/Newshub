"""
Summarization service — Gemini REST API for post summaries and digests.

Uses aiohttp to call the Gemini API directly, avoiding heavy gRPC/cffi
dependencies that don't build on Python 3.15.
"""
...
# ── LiteRouter REST API endpoint ─────────────────────────────────────────────
_LITEROUTER_URL = "https://api.literouter.com/v1/chat/completions"


async def _call_literouter(prompt: str, model_name: str, max_tokens: int = 500) -> str | None:
    """
    Call the LiteRouter API with a specific model and return the generated text.
    Returns None on any error.
    """
    from config import LITEROUTER_API_KEY
    
    if not LITEROUTER_API_KEY:
        logger.warning("LITEROUTER_API_KEY not set — skipping AI call.")
        return None
    ...
    # LiteRouter free models often have tight limits.
    logger.debug("Requesting AI (%s)...", model_name)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    ...
                    logger.error("LiteRouter API error %d [%s]: %s", resp.status, model_name, error_text[:300])
                    return None
                ...
                logger.warning("LiteRouter [%s] returned empty response.", model_name)
                return None

    except Exception:
        logger.exception("LiteRouter API call failed [%s]", model_name)
        return None
...
async def call_ai_with_failover(prompt: str, max_tokens: int = 500) -> str | None:
    ...
    """
    from config import SUPPORTED_MODELS, LITEROUTER_MODEL
    
    # 1. Get active model
    active_model = await models.get_setting("active_model")
    if not active_model:
        active_model = LITEROUTER_MODEL
        
    # 2. Try active model
    result = await _call_literouter(prompt, active_model, max_tokens)
    if result:
    ...
    result = await _call_literouter(prompt, next_model, max_tokens)
    if result:
        return scrub_markdown(result)
...
async def generate_digest(posts: list[dict]) -> str:
    """
    Generate a formatted digest from a list of news posts using Gemini.
    ...
    if result:
        logger.info("LiteRouter digest generated (%d chars).", len(result))
        return result
